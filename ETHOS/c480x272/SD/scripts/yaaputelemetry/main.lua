...
function checkEvents()
  ...
  if status.telemetry.frameType ~= -1 and libs.utils.checkTransition(1,status.telemetry.flightMode) then
    ...
    local modeName = status.frame.flightModes[status.telemetry.flightMode]
    if modeName ~= nil and status.wpEnabledModeList[string.upper(modeName)] == 1 then
...
local function task4(now)
  ...
  if status.frame.flightModes then
    status.strFlightMode = status.frame.flightModes[status.telemetry.flightMode] or ("Mode"..status.telemetry.flightMode)
    if status.telemetry.simpleMode > 0 then
...
local function configure(widget)
  ...
  form.addSourceField(line, nil, function() return status.conf.linkStatusSource4 end, function(value) status.conf.linkStatusSource4 = value end)
    ...
    widget.screenField = form.addChoiceField(line, form.getFieldSlots(line)[0],  {{"internal module", 1}, {"external module", 3}}, function() return status.conf.telemetrySource end, function(value) status.conf.telemetrySource = value end);
